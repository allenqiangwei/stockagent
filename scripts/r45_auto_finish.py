#!/usr/bin/env python3
"""R45 auto-finish script."""
import subprocess, json, time, urllib.parse
from datetime import datetime

ROUND = 45
EXP_IDS = [3341, 3342, 3343, 3344]
...
def main():
    print(f"[{datetime.now()}] R45 monitor started", flush=True)
    while True:
        all_done = True
        parts = []
        for eid in EXP_IDS:
    ...
    summary = f"R45: {total_done} strategies, {total_stda} StdA+ ({total_stda/max(total_done,1)*100:.1f}%). Best: {best_name} (score={best_score:.3f})"
    ...
    with open('/tmp/r45_summary.json','w') as f:
    ...
    print(f"[{datetime.now()}] R45 complete. Summary: /tmp/r45_summary.json", flush=True)
