#!/usr/bin/env python3
"""R542: ATR0.0875 deep grid at TP2.8, TP extreme boundary (2.85-3.0), ATR0.087 vs 0.0875 head-to-head."""
...
# ============================================================
# GRID 1: ATR0.0875 Deep Grid at TP2.8
# Goal: Full parameter exploration at the new ATR threshold
# ============================================================
print("\n=== GRID 1: ATR0.0875 Deep at TP2.8 ===")
grid1 = []
for dip in [-2.8, -2.9, -3.0, -3.1, -3.2]:
    for tp in [2.6, 2.7, 2.8, 2.9]:
        for sell_name, sell_conds in [('aR2vF2', aR2vF2), ('aR3vF2', aR3vF2)]:
            name = f"RSI18_ATR0.0875_{sell_name}_DIP{dip}_TP{tp}_MHD7"
            buy = dip_buy(dip, atr_thresh=0.0875)
...
for i, (name, buy, sell, exit_cfg) in enumerate(grid1):
    ...
    if (i+1) % 10 == 0: print(f"  Progress: {i+1}/{len(grid1)}")
print(f"Grid 1 done: {len([r for r in all_results.values() if 'ATR0.0875' in r['name']])} results")

# ============================================================
# GRID 2: TP Extreme Boundary (2.85-3.0) at ATR0.0875
# Goal: How far can TP go with ATR0.0875?
# ============================================================
print("\n=== GRID 2: TP Extreme 2.85-3.0 at ATR0.0875 ===")
grid2 = []
for tp in [2.85, 2.9, 2.95, 3.0]:
...
# ============================================================
# GRID 3: ATR0.087 vs ATR0.0875 Head-to-Head at TP2.7-2.85
# Goal: Which ATR is truly optimal at high TP?
# ============================================================
print("\n=== GRID 3: ATR Head-to-Head ===")
grid3 = []
for atr in [0.087, 0.0875]:
    for tp in [2.7, 2.75, 2.8, 2.85]:
        for dip in [-2.9, -3.0]:
            for sell_name, sell_conds in [('aR2vF2', aR2vF2), ('aR3vF2', aR3vF2)]:
                name = f"RSI18_ATR{atr}_{sell_name}_DIP{dip}_TP{tp}_MHD7_h2h"
...
for i, (name, buy, sell, exit_cfg) in enumerate(grid3):
    r = run_bt(name, buy, sell, exit_cfg)
    if r: all_results[name] = r
    if (i+1) % 8 == 0: print(f"  Progress: {i+1}/{len(grid3)}")
print(f"Grid 3 done")

# ============================================================
# GRID 4: ATR0.0875 MHD sweep at TP2.8
# Goal: Which MHD is optimal for ATR0.0875 TP2.8?
# ============================================================
print("\n=== GRID 4: ATR0.0875 MHD at TP2.8 ===")
grid4 = []
for mhd in [5, 7, 10, 14]:
    for dip in [-2.9, -3.0, -3.1]:
        name = f"RSI18_ATR0.0875_aR2vF2_DIP{dip}_TP2.8_MHD{mhd}"
        buy = dip_buy(dip, atr_thresh=0.0875)
        exit_cfg = {'take_profit_pct':2.8, 'stop_loss_pct':-10, 'max_hold_days':mhd}
...
# ============================================================
# GRID 5: RSI Period at ATR0.0875 TP2.8
# Goal: Is RSI18 still optimal at ATR0.0875?
# ============================================================
print("\n=== GRID 5: RSI Period at ATR0.0875 TP2.8 ===")
grid5 = []
for rsi in [15, 16, 17, 18, 19, 20]:
    for dip in [-2.9, -3.0]:
        name = f"RSI{rsi}_ATR0.0875_aR2vF2_DIP{dip}_TP2.8_MHD7"
        buy = dip_buy(dip, rsi_period=rsi, atr_thresh=0.0875)
        exit_cfg = {'take_profit_pct':2.8, 'stop_loss_pct':-10, 'max_hold_days':7}
...
# ============================================================
# GRID 6: DIP Fine Sweep at ATR0.0875 TP2.8
# Goal: Is DIP-2.9 still optimal at ATR0.0875?
# ============================================================
print("\n=== GRID 6: DIP Fine at ATR0.0875 TP2.8 ===")
grid6 = []
for dip in [-2.6, -2.7, -2.8, -2.9, -3.0, -3.1, -3.2, -3.3]:
    for tp in [2.7, 2.8]:
        name = f"RSI18_ATR0.0875_aR2vF2_DIP{dip}_TP{tp}_MHD7_dipfine"
        buy = dip_buy(dip, atr_thresh=0.0875)
        exit_cfg = {'take_profit_pct':tp, 'stop_loss_pct':-10, 'max_hold_days':7}
...
print(f"\n{'='*60}")
print(f"R542 COMPLETE: {valid} valid configs, {stda} StdA+ ({100*stda/max(valid,1):.1f}%)")
...
for atr in [0.087, 0.0875]:
    ...
    if matches:
        ...
        print(f"  ATR{atr}: {stda_count}/{len(matches)} StdA+, avg wr={avg_wr:.1f}%, best={best['score']} ({best['name'][:60]})")

# TP analysis at ATR0.0875
print(f"\n--- TP Analysis at ATR0.0875 ---")
for tp in [2.6, 2.7, 2.75, 2.8, 2.85, 2.9, 2.95, 3.0]:
    tp_str = str(tp)
    matches = [r for r in all_results.values() if f'_TP{tp_str}_' in r['name'] and 'ATR0.0875' in r['name']]
    if matches:
        stda_count = sum(1 for r in matches if r['stda'])
        avg_wr = sum(r['wr'] for r in matches) / len(matches)
        best = max(matches, key=lambda x: x['score'])
...
# MHD at ATR0.0875 TP2.8
print(f"\n--- MHD at ATR0.0875 TP2.8 ---")
for mhd in [5, 7, 10, 14]:
    matches = [r for r in all_results.values() if f'_MHD{mhd}' in r['name'] and 'ATR0.0875' in r['name'] and '_TP2.8_' in r['name']]
    if matches:
        stda_count = sum(1 for r in matches if r['stda'])
        best = max(matches, key=lambda x: x['score'])
        print(f"  MHD{mhd}: {stda_count}/{len(matches)} StdA+, best={best['score']}")

# RSI at ATR0.0875 TP2.8
print(f"\n--- RSI at ATR0.0875 TP2.8 ---")
for rsi in [15, 16, 17, 18, 19, 20]:
    matches = [r for r in all_results.values() if f'RSI{rsi}_' in r['name'] and 'ATR0.0875' in r['name'] and '_TP2.8_' in r['name']]
    if matches:
        stda_count = sum(1 for r in matches if r['stda'])
        best = max(matches, key=lambda x: x['score'])
...
print(f"\n--- DIP at ATR0.0875 ---")
for dip in [-2.6, -2.7, -2.8, -2.9, -3.0, -3.1, -3.2, -3.3]:
    matches = [r for r in all_results.values() if f'DIP{dip}_' in r['name'] and 'ATR0.0875' in r['name']]
    if matches:
        stda_count = sum(1 for r in matches if r['stda'])
        best = max(matches, key=lambda x: x['score'])
        print(f"  DIP{dip}: {stda_count}/{len(matches)} StdA+, best={best['score']}")

# Save results
with open('/tmp/r542_results.json', 'w') as f:
    json.dump(all_results, f, indent=2)
print(f"\nResults saved to /tmp/r542_results.json")
