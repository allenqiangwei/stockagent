#!/usr/bin/env python3
"""R538: ATR ultra-fine sweep (0.082-0.088), ATR0.085 deep grid, ATR0.085+ULCER, ATR0.085+RSI periods, ATR0.085+DIP wide."""
...
def dip_buy(dip, rsi_lb, rsi_ub, rsi_period=18, atr_thresh=0.09):
    return [
...
def dip_buy_ulcer(dip, rsi_lb, rsi_ub, rsi_period=18, atr_thresh=0.09, ulcer_thresh=11):
    return [
        {'field':'RSI','params':{'period':rsi_period},'operator':'>','compare_type':'value','compare_value':rsi_lb},
        {'field':'RSI','params':{'period':rsi_period},'operator':'<','compare_type':'value','compare_value':rsi_ub},
        {'field':'ATR','params':{'period':14},'operator':'<','compare_type':'value','compare_value':atr_thresh},
...
# ============================================================
# GRID 1: ATR Ultra-Fine Sweep (0.082-0.088 step 0.001)
# R537: ATR0.085=0.8594 champion, ATR0.080=0, ATR0.090=0.8562
# Find exact optimal between 0.082-0.088
# 7 ATR x 2 dips x 2 TPs = 28 configs
# ============================================================
print("\n" + "="*60)
print("GRID 1: ATR Ultra-Fine Sweep (0.082-0.088)")
print("="*60)
results = []
for atr_t in [0.082, 0.083, 0.084, 0.085, 0.086, 0.087, 0.088]:
    for dip in [-2.9, -3.0]:
        for tp in [2.3, 2.4]:
            buy = dip_buy(dip, 50, 75, rsi_period=18, atr_thresh=atr_t)
...
all_results['atr_ultra_fine'] = results

# ============================================================
# GRID 2: ATR0.085 Deep Grid — all DIP×TP×sell combos
# Explore full parameter space at champion ATR threshold
# 5 dips x 4 TPs x 3 sells = 60 configs
# ============================================================
print("\n" + "="*60)
print("GRID 2: ATR0.085 Deep Grid (DIP×TP×Sell)")
print("="*60)
results = []
for dip in [-2.7, -2.8, -2.9, -3.0, -3.1]:
    for tp in [2.0, 2.3, 2.35, 2.4]:
        for sell, sell_name in [(aR2vF2,'aR2vF2'), (aR3vF2,'aR3vF2'), (vF2only,'vF2only')]:
            buy = dip_buy(dip, 50, 75, rsi_period=18, atr_thresh=0.085)
            exit_cfg = {'stop_loss_pct':-20, 'take_profit_pct':tp, 'max_hold_days':7}
            name = f'RSI18_ATR0.085_{sell_name}_DIP{dip}_TP{tp}_MHD7'
...
all_results['atr085_deep'] = results

# ============================================================
# GRID 3: ATR0.085 + ULCER<11 Combined
# Combine ATR0.085 (score champion) with ULCER<11 (safety champion)
# 5 dips x 3 TPs x 2 sells = 30 configs
# ============================================================
print("\n" + "="*60)
print("GRID 3: ATR0.085 + ULCER<11 Combined")
print("="*60)
results = []
for dip in [-2.7, -2.8, -2.9, -3.0, -3.1]:
    for tp in [2.0, 2.3, 2.4]:
        for sell, sell_name in [(aR2vF2,'aR2vF2'), (aR3vF2,'aR3vF2')]:
            buy = dip_buy_ulcer(dip, 50, 75, rsi_period=18, atr_thresh=0.085, ulcer_thresh=11)
            exit_cfg = {'stop_loss_pct':-20, 'take_profit_pct':tp, 'max_hold_days':7}
            name = f'ULCER11_RSI18_ATR0.085_{sell_name}_DIP{dip}_TP{tp}_MHD7'
            r = run_bt(name, buy, sell, exit_cfg)
            if r:
                results.append(r)
                tag = ' *** StdA+ ***' if r['stda'] else ''
...
for tp in [2.0, 2.3, 2.4]:
    ...
    if rp:
        ...
        best_rp = max(rp, key=lambda x: x['score'])
        avg_dd = sum(r['dd'] for r in rp)/len(rp)
        print(f"  TP{tp}: {len(rp)} valid, {rp_stda} StdA+, best sc={best_rp['score']}, avg dd={avg_dd:.1f}%")
all_results['atr085_ulcer11'] = results

# ============================================================
# GRID 4: ATR0.085 + RSI Period Sweep (15-19)
# R537 used RSI18. Verify RSI18 is still optimal at ATR0.085
# 5 RSI x 2 dips x 2 TPs = 20 configs
# ============================================================
print("\n" + "="*60)
print("GRID 4: ATR0.085 + RSI Period Sweep (15-19)")
print("="*60)
results = []
for rsi_p in [15, 16, 17, 18, 19]:
    for dip in [-2.9, -3.0]:
        for tp in [2.3, 2.4]:
            buy = dip_buy(dip, 50, 75, rsi_period=rsi_p, atr_thresh=0.085)
            exit_cfg = {'stop_loss_pct':-20, 'take_profit_pct':tp, 'max_hold_days':7}
            name = f'RSI{rsi_p}_ATR0.085_DIP{dip}_TP{tp}_MHD7'
            r = run_bt(name, buy, aR2vF2, exit_cfg)
            if r:
                results.append(r)
                tag = ' *** StdA+ ***' if r['stda'] else ''
...
stda_count = sum(1 for r in results if r['stda'])
print(f"\n--- ATR0.085 + RSI Period Sweep Summary ---")
print(f"Total: {len(results)} valid, StdA+: {stda_count}")
for rsi_p in [15, 16, 17, 18, 19]:
...
all_results['atr085_rsi_period'] = results

# ============================================================
# GRID 5: ATR0.085 + DIP Wide Sweep (-2.5 to -3.3)
# Map the full viable DIP range at ATR0.085
# 9 dips x 2 TPs = 18 configs
# ============================================================
print("\n" + "="*60)
print("GRID 5: ATR0.085 + DIP Wide Sweep (-2.5 to -3.3)")
print("="*60)
results = []
for dip in [-2.5, -2.6, -2.7, -2.8, -2.9, -3.0, -3.1, -3.2, -3.3]:
    for tp in [2.3, 2.4]:
        buy = dip_buy(dip, 50, 75, rsi_period=18, atr_thresh=0.085)
        exit_cfg = {'stop_loss_pct':-20, 'take_profit_pct':tp, 'max_hold_days':7}
        name = f'RSI18_ATR0.085_DIP{dip}_TP{tp}_MHD7'
        r = run_bt(name, buy, aR2vF2, exit_cfg)
        if r:
            results.append(r)
            tag = ' *** StdA+ ***' if r['stda'] else ''
...
stda_count = sum(1 for r in results if r['stda'])
print(f"\n--- ATR0.085 DIP Wide Sweep Summary ---")
print(f"Total: {len(results)} valid, StdA+: {stda_count}")
for dip in [-2.5, -2.6, -2.7, -2.8, -2.9, -3.0, -3.1, -3.2, -3.3]:
    ...
    if rp:
        rp_stda = sum(1 for r in rp if r['stda'])
        best_rp = max(rp, key=lambda x: x['score'])
        print(f"  DIP{dip}: {len(rp)} valid, {rp_stda} StdA+, best sc={best_rp['score']}, ret={best_rp['ret']}%, wr={best_rp['wr']}%")
all_results['atr085_dip_wide'] = results

# ============================================================
# GRID 6: ATR0.085 + MHD Sweep (3,5,7,10)
# Verify MHD convergence holds at ATR0.085
# 4 MHD x 3 dips x 2 TPs = 24 configs
# ============================================================
print("\n" + "="*60)
print("GRID 6: ATR0.085 + MHD Sweep (3,5,7,10)")
print("="*60)
results = []
for mhd in [3, 5, 7, 10]:
    for dip in [-2.8, -2.9, -3.0]:
        for tp in [2.3, 2.4]:
            ...
            name = f'RSI18_ATR0.085_DIP{dip}_TP{tp}_MHD{mhd}'
            r = run_bt(name, buy, aR2vF2, exit_cfg)
            if r:
                results.append(r)
                tag = ' *** StdA+ ***' if r['stda'] else ''
...
all_results['atr085_mhd'] = results
...
print(f"\n{'='*60}")
print(f"R538 COMPLETE — Total time: {total_time:.0f}s ({total_time/60:.1f}min)")
print(f"{'='*60}")
...
# Save results
with open('/tmp/r538_results.json','w') as f:
    json.dump(all_results, f, indent=2)
print(f"\nResults saved to /tmp/r538_results.json")
