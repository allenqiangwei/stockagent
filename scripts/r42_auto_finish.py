#!/usr/bin/env python3
"""R42 auto-finish script. Monitors, promotes, saves round record."""
import subprocess, json, time, urllib.parse, sys
from datetime import datetime

ROUND = 42
EXP_IDS = [3329, 3330, 3331, 3332]
...
def main():
    print(f"[{datetime.now()}] R42 monitor started", flush=True)
    while True:
        all_done = True
        parts = []
        for eid in EXP_IDS:
    ...
    summary = f"R42: {total_done} strategies, {total_stda} StdA+ ({total_stda/max(total_done,1)*100:.1f}%). Best: {best_name} (score={best_score:.3f})"
    ...
    with open('/tmp/r42_summary.json','w') as f:
    ...
    print(f"[{datetime.now()}] R42 complete. Summary: /tmp/r42_summary.json", flush=True)
