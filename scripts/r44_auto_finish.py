#!/usr/bin/env python3
"""R44 auto-finish script."""
import subprocess, json, time, urllib.parse
from datetime import datetime

ROUND = 44
EXP_IDS = [3337, 3338, 3339, 3340]
...
def main():
    print(f"[{datetime.now()}] R44 monitor started", flush=True)
    while True:
        all_done = True
        parts = []
        for eid in EXP_IDS:
    ...
    summary = f"R44: {total_done} strategies, {total_stda} StdA+ ({total_stda/max(total_done,1)*100:.1f}%). Best: {best_name} (score={best_score:.3f})"
    ...
    with open('/tmp/r44_summary.json','w') as f:
    ...
    print(f"[{datetime.now()}] R44 complete. Summary: /tmp/r44_summary.json", flush=True)
