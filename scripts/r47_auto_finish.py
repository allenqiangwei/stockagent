#!/usr/bin/env python3
"""R47 auto-finish script — final grid search round."""
import subprocess, json, time, urllib.parse
from datetime import datetime

ROUND = 47
EXP_IDS = [3349, 3350, 3351]
...
def main():
    print(f"[{datetime.now()}] R47 monitor started", flush=True)
    while True:
        all_done = True
        parts = []
        for eid in EXP_IDS:
    ...
    summary = f"R47 (FINAL GRID): {total_done} strategies, {total_stda} StdA+ ({total_stda/max(total_done,1)*100:.1f}%). Best: {best_name} (score={best_score:.3f})"
    api_post('lab/exploration-rounds', {
        "round_number": ROUND, "mode": "auto",
        "started_at": STARTED_AT, "finished_at": finished_at,
        "experiment_ids": EXP_IDS, "total_experiments": 3,
    ...
    with open('/tmp/r47_summary.json','w') as f:
    ...
    print(f"[{datetime.now()}] R47 complete. Summary: /tmp/r47_summary.json", flush=True)
