#!/usr/bin/env python3
"""R46 auto-finish script."""
import subprocess, json, time, urllib.parse
from datetime import datetime

ROUND = 46
EXP_IDS = [3345, 3346, 3347, 3348]
...
def main():
    print(f"[{datetime.now()}] R46 monitor started", flush=True)
    while True:
        all_done = True
        parts = []
        for eid in EXP_IDS:
    ...
    summary = f"R46: {total_done} strategies, {total_stda} StdA+ ({total_stda/max(total_done,1)*100:.1f}%). Best: {best_name} (score={best_score:.3f})"
    ...
    with open('/tmp/r46_summary.json','w') as f:
    ...
    print(f"[{datetime.now()}] R46 complete. Summary: /tmp/r46_summary.json", flush=True)
