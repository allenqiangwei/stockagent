#!/usr/bin/env python3
"""R41 auto-finish script. Monitors, promotes, saves round record."""
import subprocess, json, time, urllib.parse, sys
from datetime import datetime

ROUND = 41
EXP_IDS = [3324, 3325, 3326, 3327, 3328]
...
def main():
    print(f"[{datetime.now()}] R41 monitor started", flush=True)
    while True:
        all_done = True
        parts = []
        for eid in EXP_IDS:
    ...
    summary = f"R41: {total_done} strategies, {total_stda} StdA+ ({total_stda/max(total_done,1)*100:.1f}%). Best: {best_name} (score={best_score:.3f})"
    api_post('lab/exploration-rounds', {
        "round_number": ROUND, "mode": "auto",
        "started_at": STARTED_AT, "finished_at": finished_at,
        "experiment_ids": EXP_IDS, "total_experiments": 5,
    ...
    with open('/tmp/r41_summary.json','w') as f:
    ...
    print(f"[{datetime.now()}] R41 complete. Summary: /tmp/r41_summary.json", flush=True)
