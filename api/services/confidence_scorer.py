...
def _build_training_data(db: Session) -> tuple[np.ndarray, np.ndarray]:
    """Build training data from executed buy plans joined with trade reviews and market regimes.
...
def predict_confidence(
    ...
    gamma_score = float(gamma) if gamma is not None else 0.0
    has_gamma = 1.0 if gamma is not None else 0.0
