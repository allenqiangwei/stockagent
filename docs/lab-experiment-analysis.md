...
> 更新时间: 2026-02-24 | 总实验: 2465(ID) / 2150+(有效) | 总策略: **1,008个** (StdA+标准) | 组合策略: 已实现 | 规则引擎P4: 已完成 | 克隆调参网格搜索: 已实现 | **29轮探索** | R29: 298实验(PSAR+RSI+KDJ 激进版A深搜), ⏳进行中 | R28新发现: **PSAR+RSI+KDJ 激进版A TP1=+367.4%回报全局最高**, PSAR+MACD+KDJ TP1=score 0.825/+258.9%/dd5.6%, UltimateOsc DD=1.24%
>
> **StdA+标准(2026-02-23起)**: score≥0.70, return>20%, dd<25%, trades≥50 (旧标准: score≥0.65, ret>10%, dd<30%)
...
**状态**: 318实验提交, ~94已完成, ~224处理中 (fast_process.py + auto_finish.py后台运行)
**方向**: 测试TP1-3, SL1-15, MHD3-15极端参数边界, 覆盖11个策略族
**实验范围**: E1850-E2167

### 已完成族结果

| 族 | 完成 | StdA | StdA率 | 最佳分 | 最佳回报 | 最低DD |
|---|---|---|---|---|---|---|
| 三指标共振 | 60/60 | 60/60 | **100%** | **0.803** | **+188.6%** | 11.2% |
| PSAR三重确认 | 31/~30 | 0/31 | 0% | 0.641 | +9.3% | 2.2% |

### 关键发现

1. **三指标共振 TP1 = 历史最佳**: score=0.803(前最佳0.769), return=+188.6%(前最佳+151.7%)
   - TP1 MHD3 组合全部 score >= 0.80 (6个), 这是首次突破0.80
   - SL对得分影响极小(SL5-15, score 0.800-0.803), TP1 MHD3是决定性参数
   - MHD3 > MHD5 > MHD7 > MHD10 > MHD15, 持仓越短越优
2. **PSAR三重确认 TP1/TP2/TP3 全部不达标**: 回报仅6-9%, 虽DD极低(2.2-3.1%)但不满足StdA(ret>10%)
   - 该族需要更长持仓和更高TP才能达到StdA标准

### 后台脚本

- `/tmp/r28_fast_process.py` — 处理remaining pending strategies (PID记录在/tmp/r28_fast.log)
- `/tmp/r28_auto_finish.py` — 等待完成后自动分析+promote (PID记录在/tmp/r28_finish.log)
- `/tmp/r28_analyze.py` — 随时可运行的分析脚本
- 预计完成时间: ~2026-02-24 凌晨2-4点

### Auto-Promote (interim)

- 60个三指标共振StdA策略已promoted (2026-02-23)
- 剩余StdA策略将由auto_finish.py自动promote
